# flow-b-calendar-gap-trigger

## Purpose

US-082: Authenticated weekly gap-trigger orchestration (settings window + enablement → detect → discover → generate into `pending-approval/`); ISO-week idempotency; clean no-ops; inactive n8n Schedule→HTTP export; ADR-0001 HTTP-only; no auto-publish / no LinkedIn API publish; does not implement US-076–US-081 internals.
