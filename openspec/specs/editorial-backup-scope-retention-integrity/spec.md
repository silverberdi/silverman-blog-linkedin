# editorial-backup-scope-retention-integrity

## Purpose

Operator-visible contracts for editorial-state backup **scope**, **retention**, and **integrity verification** (BL-014 / US-036): what must and must not be backed up, package layout under `metadata/backups/`, retention bounds, automated integrity checks with secret-safe `pass` / `fail` / `blocked` outcomes, and an explicit boundary that restoration testing and recovery procedures belong to US-037.
