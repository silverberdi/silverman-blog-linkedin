# flow-a-n8n-workflow-activation

## Purpose

Controlled activation of the canonical Flow A n8n workflow on the Ubuntu server: Schedule Trigger daily 09:00 UTC, single-flight concurrency guard, restart/recovery evidence, and operator-visible PASS/PENDING/FAIL verification — while keeping the repository export `"active": false` and excluding LinkedIn publication enablement (US-011 / BL-005).
