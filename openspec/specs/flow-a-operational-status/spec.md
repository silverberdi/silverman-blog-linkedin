...
Read-only aggregation contract for Flow A operational observability (BL-010 / US-026): authenticated `GET /flow-a/operational-status`, deterministic execution/campaign/calendar/LinkedIn classifications from confined on-disk evidence, partial-result handling, safe output, and zero-mutation guarantees. Does not implement US-027 stage durations, BL-011 alerting, or BL-015 UI.
