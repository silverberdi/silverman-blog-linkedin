...
Flow A ready-post editorial validation gate for the `silverman-blog-linkedin` worker: file location and extension checks, slug and image validation, frontmatter rules, blocking content checks, non-blocking editorial warnings, campaign metadata integration, and structured `ReadyPostValidationResult` output. Implements child slice 3 under umbrella `flow-a-automatic-blog-linkedin-publishing-roadmap`.
