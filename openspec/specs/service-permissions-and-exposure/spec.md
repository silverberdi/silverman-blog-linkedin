...
Operator-visible normative service permissions and exposure review for **BL-026 / US-062 + US-063**: least privilege, open ports, authentication, allowed paths, secrets separation (ratifying US-058), and documented accepted exposure (LAN-first worker/n8n; Authority Manager public exposure only as front-only UI under US-099 with private worker API; exceptional LinkedIn OAuth callback via Cloudflare for reauth only; Comfy Cloud and DeepSeek as outbound API clients; Google identity/allowlist and operator JWT console→API under BL-035 / US-097/US-098) — without publishing the worker API on the public internet, LinkedIn enablement mutation, or a network-security platform.
