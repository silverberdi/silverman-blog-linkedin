# linkedin-draft-review-writing

## Purpose

Authenticated `POST /write-linkedin-draft` endpoint for persisting one LinkedIn review draft Markdown file under `linkedin-posts/review/` from client-supplied content, writing run metadata under `metadata/runs/` (summary fields only), and returning structured JSON for n8n branching—without AI providers, publishing, or mutating source blog files.
