...
Operator-visible policy for Flow A LinkedIn variant publication expectations — strategy-driven publish default (all scheduled variants unless operator overrides), optional supervision window while `pending`, when review is mandatory (Flow B only), blocked/deferred states, and how the policy informs BL-007 eligibility without implementing auto-queue or worker publication behavior (US-015 / BL-006 story 1).
