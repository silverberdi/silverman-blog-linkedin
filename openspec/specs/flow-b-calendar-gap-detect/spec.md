...
Authenticated detect-only next-week LinkedIn calendar gap sensor; settings-driven timezone / `min_lead_days` / threshold; structured `gaps[]` result; no campaign/draft mutation; detect allowed when trigger disabled. Gap trigger/discovery/draft/approve remain out of scope (US-078–US-082).
...
This capability MUST NOT implement gap trigger orchestration (US-082), AI discovery/draft (US-078/US-079), blog approve/promote (US-080/US-081), or LinkedIn API publication. Fail-closed auto-trigger semantics for `gap_trigger_enabled` remain reserved for US-082.

#### Scenario: No trigger or draft routes required

- **WHEN** this capability’s change tasks are completed
- **THEN** gap-trigger and draft-generation endpoints are not required to exist
- **AND** detect remains non-mutating and secret-safe
