...
Normative classification of LinkedIn publication outcomes (recoverable / non-recoverable / uncertain / blocked) and safe recovery paths for the `silverman-blog-linkedin` worker (BL-008 / US-021). Consumes US-019 failure evidence read-only; defines operator-visible recovery policy without changing publication execution, endpoints, or evidence shapes. Retry limits, attempt counting, automated retry, and evidence preservation across manual re-queue remain US-022.
