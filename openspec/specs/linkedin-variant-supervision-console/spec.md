...
Operator-facing console for Flow A LinkedIn variants in the optional `pending` supervision window (BL-015 / US-038 Story 1 + US-039 Story 2 + US-040 Story 3): authenticated `GET /flow-a/linkedin-variants/pending-supervision` aggregation over campaign metadata and editorial calendar (including nullable `draft_content`, deferred eligibility, and secret-safe `integration_failures[]`), same-origin static console at `GET /flow-a/console/linkedin-variant-supervision`, edit/defer/cancel actions via existing US-017 POSTs, blocked-state display context (enablement, deferred capabilities, integration failures), and secrets-safe HTML. LinkedIn API publish paths and BL-007 auto-queue implementation changes remain out of scope for this capability.
