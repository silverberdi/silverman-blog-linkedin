...
Operator-facing read-only console for Flow A LinkedIn variants in the optional `pending` supervision window (BL-015 / US-038 Story 1): authenticated `GET /flow-a/linkedin-variants/pending-supervision` aggregation over campaign metadata and editorial calendar, same-origin static console at `GET /flow-a/console/linkedin-variant-supervision`, failure/enablement display context, and secrets-safe HTML. Edit/defer/cancel UI (US-039/US-040), LinkedIn API publish paths, and BL-007 auto-queue changes are out of scope for this capability.
