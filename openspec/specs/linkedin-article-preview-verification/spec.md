# linkedin-article-preview-verification Specification

## Purpose

On-demand verification of LinkedIn article preview inputs (title, description, public image availability) for one Flow A campaign with a generated LinkedIn package — comparing package metadata, public-checkout front matter, and the live public site's Open Graph tags and image URL over HTTP, with stable outcome codes and dry-run-safe evidence handling. Confirming actual LinkedIn rendering and cache/metadata diagnosis is US-024 and out of scope.
