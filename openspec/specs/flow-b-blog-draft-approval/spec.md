...
This capability MUST NOT implement promote/move to `blog-posts/ready/`, spill algorithm A, or gap trigger orchestration (US-081 / US-082). It MUST NOT introduce a revision-history CMS or mandatory edit-apply loop. It MUST NOT change the HTTP contracts of `flow-b-blog-draft-generation` (`POST /flow-b/generate-blog-drafts`), `flow-b-topic-discovery`, `flow-b-calendar-gap-detect`, or `flow-b-gap-operator-settings` except by reading/updating pending-approval artifacts produced by draft generation.

#### Scenario: No promote or trigger required for US-080 completion

- **WHEN** this capability's requirements are evaluated
- **THEN** promote-to-`ready/`, spill algorithm A, and gap trigger endpoints are not required for US-080 completion
