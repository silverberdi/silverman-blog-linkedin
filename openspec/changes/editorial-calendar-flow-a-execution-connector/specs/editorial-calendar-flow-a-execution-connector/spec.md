...
- **THEN** `publish_blog_post`, `generate_linkedin_package`, and `schedule_linkedin_distribution` are invoked in order for that item, package receives publish result identifiers, schedule receives package result identifiers, and the item `execution_status` is `executed` on success
