...
- Remain an active, long-running OpenSpec change while child changes are proposed, applied, and archived—not archived immediately after stakeholder approval of planning artifacts.
...
- Archiving this change before Flow A child changes are complete, validated, or intentionally superseded.
...
- Define success criteria for completed Flow A.
...
- **Dependencies**: Child changes depend on this umbrella for policy and sequencing. This umbrella stays active (not archived) while those child changes execute. Preferred order: editorial canon → lifecycle/idempotency → validation → blog publish endpoint → derivative package → scheduling → n8n orchestration → LinkedIn API integration (deferred). See design.md and tasks.md.
