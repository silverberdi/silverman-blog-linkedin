...
**Migration**: Replace ready-only optional backfill semantics with active-folder backfill required for validation; failed backfill blocks publish explicitly with `blog_image_active_sibling_backfill_failed`.
...
**Migration**: Classify failed active-folder backfill as explicit `blog_image_active_sibling_backfill_failed` with `retryable` or `repair_required` classification; do not allow publish without local companion when validation requires it.
