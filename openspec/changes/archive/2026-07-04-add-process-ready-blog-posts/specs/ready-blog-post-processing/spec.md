...
The worker SHALL require valid API key authentication for `POST /process-ready` using the configured `SILVERMAN_BLOG_LINKEDIN_API_KEY`.
...
- **WHEN** a client sends `POST /process-ready` without an `Authorization` header
- **THEN** the worker responds with HTTP `401` and a JSON body that does not expose secret values

#### Scenario: Invalid API key

- **WHEN** a client sends `POST /process-ready` with an incorrect Bearer token
...
- **WHEN** authentication fails for `POST /process-ready`
