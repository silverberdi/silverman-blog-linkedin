...
**Script behavior:** Read-only; no secrets printed; no n8n activation; no LinkedIn API calls; no deploy/restart. Resolves base path with explicit source reporting; checks worker `/health` and `/openapi.json` Flow A paths; lists latest `metadata/runs/`, `metadata/campaigns/`, `linkedin-posts/generated/` editorial artifacts; lists published `_posts` and `assets/images` matches under the public GitHub Pages repo host mount (`PUBLIC_BLOG_HOST_MOUNT`); exports n8n workflows and confirms `silvermanFlowAPublish01` is inactive with 26 nodes. Overall `PASS` / `PENDING` / `FAIL` semantics gate on worker health, public blog repo readiness, n8n inactive state, and presence of editorial smoke artifacts (campaign/generated); published blog file matches are informational.
...
**Evidence path correction (2026-07):** Published Jekyll files (`_posts/`, `assets/images/`) are written to the public GitHub Pages repo checkout (`/public-blog` in container; host mount e.g. `/home/silverman/silverberdi.github.io`), not the editorial workspace. `collect-flow-a-smoke-evidence.sh` separates **Editorial artifacts** (metadata, generated LinkedIn) from **Public blog artifacts** (slug-matched published files under `PUBLIC_BLOG_HOST_MOUNT`). Published blog matches are informational for diagnosis only; they do not affect `OVERALL: PASS`.

**Evidence PASS semantics correction (2026-07):** After commit `07ce499`, evidence collection could report `OVERALL: PASS` when campaign metadata existed alongside public blog files, even though Flow A had not reached `distribution_scheduled` and `linkedin_package` / `linkedin_distribution` were absent. `OVERALL: PASS` now requires worker/public/n8n readiness **and** campaign evidence of `distribution_scheduled` (or later) or `linkedin_distribution`. Campaign state `error` yields `OVERALL: FAIL`. Intermediate states (`validated`, `blog_published`, `derivatives_generated` without distribution) yield `OVERALL: PENDING`.
...
**Worker fix:** When public targets exist at expected paths, idempotency key matches, source path/hash align, and public file content matches the current ready post/image, reconcile campaign metadata to `blog_published` with `blog_publish.status: reconciled` instead of failing — including when campaign state is `error` from prior failed publish attempts (`blog_publish_invalid_campaign_state`, `blog_publish_target_exists`, `blog_publish_public_repo_not_configured`). Reconciliation from `error` clears only those stale publish-related error codes; unrelated errors are preserved. Unsafe mismatches still fail strictly.

**Third root cause (2026-07, commit `8b656b8`):** After error-state reconciliation shipped, smoke still failed with `blog_publish_target_exists` and `source_public_url: null` in the worker response while campaign metadata already held `source_public_url` and stale `blog_publish_target_exists`. Reconciliation ordering still allowed the overwrite guard to win when the initial public-target probe did not align with campaign resume context (preflight not enriched from campaign metadata on skip-validation paths). Fix: merge campaign publication identity into preflight before target checks; call unified reconciliation before any `blog_publish_target_exists` return and again when `run_publish` refuses overwrite; expose `blog_publish.reconciliation_skip_reason` when recovery is skipped; preserve or compute `source_public_url` on failed responses. Flow A remains incomplete until worker smoke reaches `distribution_scheduled` with `linkedin_distribution`.

**Fourth root cause (2026-07, commit `b8b7a7a`):** After reconciliation ordering shipped, smoke still failed with `blog_publish_target_exists` and `blog_publish.reconciliation_skip_reason: blog_publish_reconciliation_skipped_public_content_mismatch` while public files existed and `source_public_url` was present. Reconciliation compared public artifacts against a render path that was not guaranteed identical to `apply_plan` / `run_publish`, and could not distinguish post-body drift from ready-image drift (campaign hash covers Markdown only). Fix: centralize `render_expected_public_post` for publish and reconciliation; compare public post to that canonical output (not raw ready markdown); add separate image mismatch reason; expose expected/actual SHA-256 diagnostics without full content in HTTP responses. Unsafe overwrite remains forbidden.

**Fifth root cause (2026-07):** After canonical publish-output comparison shipped, smoke still failed with `blog_publish_target_exists` and `blog_publish.reconciliation_skip_reason: blog_publish_reconciliation_skipped_public_image_mismatch` while the public post matched canonical output and a public image existed at the expected path. The public image had been manually corrected in the real GitHub Pages repo after initial publish; the ready-folder PNG was an older reuse and must not overwrite the official public asset. Fix: when canonical post matches and the public image exists at the exact expected target path, reconcile by **adopting** the existing public image as the published asset (`public_image_adopted`, `public_image_source: existing_public_asset`, ready/published image SHA-256 recorded). Guardrails: canonical post must match; exact expected image path; Flow A only; recoverable campaign state; no arbitrary overwrite; missing public image or post mismatch still fails strictly.

**Script behavior:** Calls `GET /health`, then `POST /publish-blog-post`, `POST /generate-linkedin-package`, `POST /schedule-linkedin-distribution` in order; prints campaign metadata after each step (including after publish failure when `campaign_id` is known); on publish success prints public image adoption metadata when present; verifies public blog files, generated LinkedIn artifacts, and `distribution_scheduled` state with `linkedin_distribution` metadata; exits `OVERALL: PASS` or `OVERALL: FAIL`. No secrets, no n8n activation, no LinkedIn API, no git push, no destructive cleanup.
