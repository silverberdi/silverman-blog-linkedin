...
- **BL-007 / US-019 publication evidence formalization:** spec delta + behavioral tests + additive optional `linkedin_post_urn` / `published_at` on `auto_queue_results` entries under `auto_queue_pending`. Evidence write / failure-context / blocked-vs-failed behavior was already present from US-018; this slice makes the contract normative and surfaces URN/`published_at` on auto-queue result entries. **Implemented, not deployed** (local build only until an approved deploy). BL-007 remains open; US-019 is **in progress** (not closed); US-020 cadence/sequence remains deferred. No RUNTIME-STATE change (live flags untouched). Operator evidence contract: [linkedin-publication-prerequisites.md](deployment/linkedin-publication-prerequisites.md#publication-evidence-and-failure-taxonomy-us-019).
