...
- **42** canonical OpenSpec specs (strict validation passing after US-027 sync of `flow-a-operational-status`; US-028 alerts delta pending sync/archive)
- **1,097** automated tests collected after US-028 operational-alerts implementation (11 focused alerts tests + prior suite)
...
Key endpoints include `GET /health`, read-only `GET /flow-a/operational-status`, authenticated `POST /flow-a/operational-alerts/evaluate` (US-028), Flow A (`POST /publish-blog-post`, `/generate-linkedin-package`, `/schedule-linkedin-distribution`, `/complete-flow-a-ready-path`, calendar connector), LinkedIn publication (`/queue-linkedin-publication`, `/publish-linkedin-due-variants`, `/cancel-linkedin-publication`), and Flow B-adjacent draft endpoints (`/process-ready`, `/process-file`, `/generate-linkedin-draft`).
...
- Flow A operational alerts (**US-028 / BL-011 story 1**) — **implemented and tested locally; controlled-fixture demonstrated, not deployed or live operationally validated**. Authenticated `POST /flow-a/operational-alerts/evaluate` derives secret-safe US-028 candidates from the same confined operational-status evidence used by `GET /flow-a/operational-status`: `item_moved_to_error` (error-folder / failed campaign location), `image_generation_failure` (`comfyui` bucket), and `blog_publication_failure` (`blog_publish_*` / `blog_git_publication_*` only; LinkedIn-preview checkout codes excluded). Optional fail-closed generic webhook emission behind `SILVERMAN_FLOW_A_OPERATIONAL_ALERTS_ENABLED` + `SILVERMAN_FLOW_A_OPERATIONAL_ALERTS_WEBHOOK_URL`; idempotent ledger only at `metadata/operational-alerts/emissions.json` after HTTP 2xx. Evaluate-only does not mutate campaign/run/editorial lifecycle. No Slack/email/UI SDK; no US-029/US-030 types; no BL-015 UI; no deploy/live mutation as part of this change. Focused suite: 11 alerts tests + operational-status regression (28) pass. Operator contract: [flow-a-operational-alerts.md](operations/flow-a-operational-alerts.md). US-028 business acceptance and BL-011 remain open; BL-010 remains open and unchanged by this work.
- Flow A operational status (**US-026 + US-027 / BL-010 slices 1–2**) — **implemented and tested locally; controlled-fixture demonstrated, not deployed or live operationally validated**. Authenticated read-only `GET /flow-a/operational-status` consolidates persisted run outcomes, independent campaign lifecycle/blocked/stale/in-progress flags, delayed calendar items, and descriptive LinkedIn schedule/publication evidence (US-026), plus stage durations derived read-only from existing `state_history` / run clocks (closed and observation-relative open intervals, execution `duration_seconds`, supplemental `attempt_duration_seconds`) and dependency-failure aggregation of validated error codes into `comfyui` / `deepseek` / `linkedin` / `github_pages_checkout` / `unclassified` buckets without calling any integration (US-027). No new persisted timing fields. Fixed confined sources, stable partial-result issues, deterministic supplied time/order, safe output, and byte-for-byte zero mutation are covered by focused tests. Operator contract and fixture demonstration: [flow-a-operational-status.md](operations/flow-a-operational-status.md). US-026 and US-027 business review/acceptance remain pending; BL-010 remains open. Status observation does not send notifications, call alert webhooks, or write `metadata/operational-alerts/` — alerting is owned by US-028.
