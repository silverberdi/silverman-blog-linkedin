...
- **BL-015** Flow A LinkedIn variant supervision console (US-038–US-040, plus variants US-040A–US-040E) — **US-038 (Story 1) + US-039 (Story 2) + US-040 (Story 3) implemented in worker (not Story accepted; not BL-015 closed; not unattended production):** authenticated `GET /flow-a/linkedin-variants/pending-supervision` aggregates `publish_state=pending` variants (calendar join, nullable `draft_content`, deferred/`auto_queue_eligible`/`operator_supervision_reason`, `issues[]`, secret-safe `integration_failures[]` for failed siblings); same-origin console at `GET /flow-a/console/linkedin-variant-supervision` lists pending rows and offers edit/defer/cancel that call existing US-017 `POST /correct-linkedin-variant`, `POST /defer-linkedin-variant`, and `POST /cancel-linkedin-publication` (dry-run default on; confirm for real cancel; enablement display-only). **US-040A implemented in worker (console-layer only; not Story accepted; not BL-015 closed):** React + TypeScript + Vite app under `frontend/linkedin-variant-supervision-console/` builds static artifacts into `src/silverman_blog_linkedin/static/linkedin-variant-supervision-console/` (worker serves `index.html` + `/assets/*`; no separate frontend server; build with `npm ci && npm run build` before Docker/`COPY src`); list remains first-class with Stories 1–3 parity; typed injectable-auth API client; shared normalized model; scaffolds for month calendar / detail / schedule / filters / status (US-040B–US-040C behavior not implemented); US-040D public URL + Google auth not activated; US-040E polish not done. No BFF/DB/user-management/public hosting. US-017 worker mechanics unchanged. See [backlog.md](product/backlog.md). Mechanics: [linkedin-variant-supervision-mechanics.md](operations/linkedin-variant-supervision-mechanics.md)
