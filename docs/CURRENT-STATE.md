...
- **BL-007 / US-019 publication evidence formalization:** spec delta + behavioral tests + additive optional `linkedin_post_urn` / `published_at` on `auto_queue_results` entries under `auto_queue_pending`. Evidence write / failure-context / blocked-vs-failed behavior was already present from US-018; this slice makes the contract normative and surfaces URN/`published_at` on auto-queue result entries. **Implemented, not deployed** (local build only until an approved deploy). BL-007 remains open; US-019 is **in progress** (not closed). No RUNTIME-STATE change (live flags untouched). Operator evidence contract: [linkedin-publication-prerequisites.md](deployment/linkedin-publication-prerequisites.md#publication-evidence-and-failure-taxonomy-us-019).
- **BL-007 / US-020 publish-time sequence and cadence guard:** per-campaign guard enforced at publish time for every publish-due evaluation over `queued` variants (plain, combined `auto_queue_pending`, targeted, cross-campaign scan) plus an auto-queue sequence pre-filter. Sequence rule (no later variant while an earlier one is awaiting publication — `pending` including operator-deferred, or `queued` unpublished; `failed`/`cancelled` release), real 72-hour cadence anchored to stored `published_at` (US-019, read-only), evidence fail-closed (`linkedin_publish_blocked_evidence_invalid`), stable reasons `linkedin_publish_blocked_sequence` / `linkedin_publish_blocked_cadence` / `linkedin_publish_auto_queue_skipped_sequence`, `publish_now` timing-only scope. **Implemented, not deployed** (local build only until an approved deploy). BL-007 remains open; US-019/US-020 are **in progress** (not closed). No RUNTIME-STATE change (live flags untouched). Operator contract: [linkedin-publication-prerequisites.md](deployment/linkedin-publication-prerequisites.md#publish-time-sequence-and-cadence-guard-us-020).
...
- **BL-007** remains open. US-018 is validated; US-019 publication-evidence formalization is **implemented (not deployed), in progress / not complete**; US-020 publish-time sequence/cadence guard is **implemented (not deployed), in progress / not complete**. Closure of US-019/US-020/BL-007 remains a separate authorized validation step. Former construction WIP was absorbed under the approved US-018 change; record: [bl-007-auto-queue-pending-handoff.md](product/bl-007-auto-queue-pending-handoff.md)
