...
- Flow A operational alerts (**US-028 + US-029 + US-030 / BL-011 stories 1–3**) — **implemented, tested locally, deployed to `192.168.0.194` (`BUILD_REVISION=b67c538`), and demonstrated with controlled live smoke 2026-07-17; not operator-accepted**. Live smoke: authenticated evaluate returned existing US-028 candidates (`item_moved_to_error`, `blog_publication_failure`); report ingest created `failed_n8n_workflow`; eight-type `summary.counts`; evaluate-only zero lifecycle mutation; emit with flags unset → `emission.status=disabled`; auth 401 without API key; smoke orchestration-failure entry removed afterward. `unhealthy_worker` not expected on healthy live folders (`folders_ready=true`; covered by local degraded fixtures). Authenticated `POST /flow-a/operational-alerts/evaluate` derives secret-safe candidates. **US-028:** `item_moved_to_error`, `image_generation_failure`, and `blog_publication_failure` from operational-status evidence (LinkedIn-preview checkout codes excluded from blog publication). **US-029:** `partial_calendar_execution` (warning), `linkedin_token_or_publication_failure` (error; preview checkout codes excluded), and `stale_campaign` (warning). **US-030:** `unhealthy_worker` from in-process `validate_folders` / health-equivalent readiness; `failed_n8n_workflow` from authenticated `POST /flow-a/operational-alerts/report-orchestration-failure` evidence under `metadata/operational-alerts/orchestration-failures.json` only. Optional fail-closed webhook behind `SILVERMAN_FLOW_A_OPERATIONAL_ALERTS_ENABLED` + webhook URL (not enabled on server). No Slack/email/UI SDK; no BL-015 UI; production n8n Error Trigger wiring not required. Operator contract: [flow-a-operational-alerts.md](operations/flow-a-operational-alerts.md). US-028, US-029, and US-030 business acceptance and BL-011 remain open.
