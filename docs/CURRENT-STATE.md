...
- LinkedIn retry and recovery classification (**US-021** / BL-008 story 1): **policy defined** — normative classification of publication outcomes (recoverable transient / recoverable after remediation / non-recoverable as-is / uncertain, keyed on `last_error_code` + `http_status`; unlisted combinations fail safe to uncertain), blocked as a separate non-failure class, token-renewal-precedes-re-queue rule, and mandatory operator verification on LinkedIn before re-queueing uncertain outcomes. Docs/spec only — no worker code, endpoints, or evidence-field changes; policy defined ≠ operationally validated. Evidence: [linkedin-retry-recovery-classification.md](operations/linkedin-retry-recovery-classification.md).

## Implemented but not operationally validated

- LinkedIn retry limits and recovery evidence (**US-022** / BL-008 story 2): **implemented and unit-tested — not deployed, not operationally validated.** Per-variant retry budget (initial + 2 manual retries; only real LinkedIn API calls count), append-only `linkedin_publication_attempts` and `linkedin_recovery_history` evidence, class-aware re-queue with `recovery_confirmation` enum on `POST /queue-linkedin-publication`, failed-content correction via `POST /correct-linkedin-variant` (content-invalid class only), and `failed → cancelled` via `POST /cancel-linkedin-publication` with full evidence preservation. The previously recorded US-021 divergence is resolved in code: manual re-queue of a `failed` variant no longer clears stored `linkedin_publication` evidence. No new endpoints, env vars, or `publish_state` values; enablement guard unchanged. BL-008 remains open pending post-implementation review and operational validation. Evidence: [linkedin-retry-recovery-classification.md](operations/linkedin-retry-recovery-classification.md), unit suites `tests/test_linkedin_publication.py` / `tests/test_linkedin_supervision_flow.py`.
- LinkedIn article preview input verification (**US-023** / BL-009 story 1): **implemented and unit-tested — not deployed, not operationally validated.** New authenticated `POST /validate-linkedin-article-preview` (dry-run default `true`) verifies article preview **inputs** for one campaign with a generated LinkedIn package: `linkedin_package.article_preview` completeness, public-checkout `_posts/` front matter consistency (skipped when `SILVERMAN_GITHUB_PAGES_REPO_PATH` unset), live `og:title`/`og:description`/`og:image` tags at `public_url`, and HTTPS availability of `public_image_url` — with stable `linkedin_preview_validation_*` codes and an additive `linkedin_article_preview_validation` evidence block on real runs only. **No LinkedIn API involvement**: no calls to `api.linkedin.com`, no OAuth token reads; passing means the inputs LinkedIn would scrape are correct, not that LinkedIn renders the preview (US-024 boundary). No new env vars; package generation, publication, and US-022 behavior unchanged. US-023 is not accepted and BL-009 stays open until operator-demonstrated validation on `192.168.0.194` against the live site. Unit suite: `tests/test_linkedin_preview_validation.py`. Operator procedure: [linkedin-publication-prerequisites.md](deployment/linkedin-publication-prerequisites.md).
- LinkedIn article preview rendering confirmation (**US-024** / BL-009 story 2): **procedure defined — not operationally validated, story not accepted; BL-009 remains open.** Normative operator procedure (docs + canonical procedure-spec only, **no worker code**, no LinkedIn API usage, no UI scraping): pre-publish LinkedIn Post Inspector confirmation and post-publish real-post observation against recorded `article_preview` metadata; cache-vs-input decision matrix keyed on the US-023 verification result with fixed outcome labels (`preview_confirmed`, `preview_stale_cache`, `preview_inputs_incorrect`, `preview_not_rendered_post_format`, `confirmation_blocked`); safe Post Inspector-only re-scrape (new posts only; no test posts, no cache-busting URLs); blocked states with next actions; per-confirmation evidence-record template (campaign metadata never edited). Procedure defined ≠ operationally validated ≠ accepted: acceptance requires an operator-demonstrated confirmation on a real campaign, which depends on the pending US-023 deploy + operational validation on `192.168.0.194`. Canonical spec `linkedin-article-preview-rendering-confirmation` lands at spec sync. Evidence: [linkedin-preview-rendering-confirmation.md](operations/linkedin-preview-rendering-confirmation.md).
- LinkedIn article preview fallback policy (**US-025** / BL-009 story 3): **policy defined — not operationally validated, story not accepted; BL-009 remains open.** Normative operator fallback policy (docs + canonical procedure-spec only, **no worker code**, no new LinkedIn API usage, no UI scraping, no automatic execution) for reacting to a wrong or missing preview: triggers limited to recorded US-024 outcomes (`preview_stale_cache` after a completed safe re-scrape cycle, `preview_not_rendered_post_format`, published post retaining a stale/incorrect card); pre-publish procedure (accept / delay via defer / correct-and-reverify / cancel) and post-publish recovery (accept-and-record default; approval-gated manual post removal; evidence mutation forbidden) over existing guarded endpoints; supported/approval-gated/forbidden classification with delete/re-post forbidden and two deferred future-change candidates (`content.article` post format; worker-supported deletion/re-publication); fixed outcome vocabulary, evidence-record template (campaign metadata never edited), and blocked states with next actions. Policy defined ≠ operationally validated ≠ accepted: acceptance requires an operator-demonstrated fallback decision on a real campaign; BL-009 remains open pending the US-023 deploy + operational validation on `192.168.0.194`, the US-024 demonstration, and the US-025 demonstration. Canonical spec `linkedin-article-preview-fallback` lands at spec sync. Evidence: [linkedin-preview-fallback-policy.md](operations/linkedin-preview-fallback-policy.md).
