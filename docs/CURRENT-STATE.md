...
- **43** canonical OpenSpec specs (strict validation passing after US-028 sync of `flow-a-operational-alerts` and status scope clarification)
- **1,108** automated tests collected after US-030 operational-alerts extension (22 focused alerts tests + prior suite)
...
Key endpoints include `GET /health`, read-only `GET /flow-a/operational-status`, authenticated `POST /flow-a/operational-alerts/evaluate` (US-028 + US-029 + US-030) and `POST /flow-a/operational-alerts/report-orchestration-failure` (US-030), Flow A (`POST /publish-blog-post`, `/generate-linkedin-package`, `/schedule-linkedin-distribution`, `/complete-flow-a-ready-path`, calendar connector), LinkedIn publication (`/queue-linkedin-publication`, `/publish-linkedin-due-variants`, `/cancel-linkedin-publication`), and Flow B-adjacent draft endpoints (`/process-ready`, `/process-file`, `/generate-linkedin-draft`).
...
- Flow A operational alerts (**US-028 + US-029 + US-030 / BL-011 stories 1–3**) — **implemented, tested locally, and deployed to `192.168.0.194` (`BUILD_REVISION=b67c538`); controlled-fixture demonstrated; not live operationally validated or operator-accepted**. Authenticated `POST /flow-a/operational-alerts/evaluate` derives secret-safe candidates. **US-028:** `item_moved_to_error`, `image_generation_failure`, and `blog_publication_failure` from operational-status evidence (LinkedIn-preview checkout codes excluded from blog publication). **US-029:** `partial_calendar_execution` (warning), `linkedin_token_or_publication_failure` (error; preview checkout codes excluded), and `stale_campaign` (warning). **US-030:** `unhealthy_worker` from in-process `validate_folders` / health-equivalent readiness (`folders_ready=false`; no `/health` HTTP loopback; no process-down or `BUILD_REVISION` mismatch alerts); `failed_n8n_workflow` from authenticated `POST /flow-a/operational-alerts/report-orchestration-failure` evidence under `metadata/operational-alerts/orchestration-failures.json` only (failed `metadata/runs/*` alone does not produce this type). `summary.counts` covers all eight types. Optional fail-closed generic webhook emission behind `SILVERMAN_FLOW_A_OPERATIONAL_ALERTS_ENABLED` + `SILVERMAN_FLOW_A_OPERATIONAL_ALERTS_WEBHOOK_URL`; idempotent ledger only at `metadata/operational-alerts/emissions.json` after HTTP 2xx. Evaluate-only does not mutate campaign/run/editorial lifecycle or the orchestration-failures store. Report writes only orchestration-failure evidence. No Slack/email/UI SDK; no BL-015 UI; production webhook enablement and n8n Error Trigger wiring not enabled as part of this deploy. Focused suite: 22 alerts tests + operational-status regression (28) pass. Operator contract: [flow-a-operational-alerts.md](operations/flow-a-operational-alerts.md). US-028, US-029, and US-030 business acceptance and BL-011 remain open; BL-010 remains open and unchanged by this work.
- Flow A operational status (**US-026 + US-027 / BL-010 slices 1–2**) — **implemented and tested locally; controlled-fixture demonstrated, not deployed or live operationally validated**. Authenticated read-only `GET /flow-a/operational-status` consolidates persisted run outcomes, independent campaign lifecycle/blocked/stale/in-progress flags, delayed calendar items, and descriptive LinkedIn schedule/publication evidence (US-026), plus stage durations derived read-only from existing `state_history` / run clocks (closed and observation-relative open intervals, execution `duration_seconds`, supplemental `attempt_duration_seconds`) and dependency-failure aggregation of validated error codes into `comfyui` / `deepseek` / `linkedin` / `github_pages_checkout` / `unclassified` buckets without calling any integration (US-027). No new persisted timing fields. Fixed confined sources, stable partial-result issues, deterministic supplied time/order, safe output, and byte-for-byte zero mutation are covered by focused tests. Operator contract and fixture demonstration: [flow-a-operational-status.md](operations/flow-a-operational-status.md). US-026 and US-027 business review/acceptance remain pending; BL-010 remains open. Status observation does not send notifications, call alert webhooks, or write `metadata/operational-alerts/` — alerting is owned by US-028 / US-029 / US-030.
