...
**`verified_at_utc`:** `2026-07-19T20:30:00Z`
**Evidence source:** Worker redeploy on Ubuntu `192.168.0.194` after US-040L archive push (src checkout `143a40b`); health 200 with calendar store ready; deploy verify OVERALL PASS; live console static assets `index-B4ePEkH6.js` / `index-DmEqOAc6.css`; container `BUILD_REVISION=143a40bd61dc…`

| Fact | Value | Evidence |
|------|-------|----------|
| Worker URL | `http://192.168.0.194:8010` | Deploy + health check |
| `BUILD_REVISION` | Image built with `BUILD_REVISION=143a40bd61dc…` at deploy; server src checkout `143a40b` (US-040L archive HEAD) | Deploy log 2026-07-19 + container env |
| Supervision console assets | **US-040L deployed** — live `index-B4ePEkH6.js` / `index-DmEqOAc6.css` (header Filters modal; no permanent filter-dock) | Worker static `index.html` on deploy target |
