...
**`verified_at_utc`:** `2026-07-19T17:12:00Z`
**Evidence source:** Worker redeploy on Ubuntu `192.168.0.194` after US-040K archive push (src checkout `0d90b3d`); health 200 with calendar store ready; OpenAPI Flow A verify OVERALL PASS (12/12); OpenAPI schemas expose `operator_timezone` on defer/reopen/blog schedule-update; live console static assets `index-CoeIIs9v.js` / `index-CfjReggH.css`; container `BUILD_REVISION=0d90b3daacee…`

| Fact | Value | Evidence |
|------|-------|----------|
| Worker URL | `http://192.168.0.194:8010` | Deploy + health check |
| `BUILD_REVISION` | Image built with `BUILD_REVISION=0d90b3daacee…` at deploy; server src checkout `0d90b3d` (US-040K push HEAD) | Deploy log 2026-07-19 + container env |
| Supervision console assets | **US-040K deployed** — live `index-CoeIIs9v.js` / `index-CfjReggH.css` | Worker static `index.html` on deploy target |
