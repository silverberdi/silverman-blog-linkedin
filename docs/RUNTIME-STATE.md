...
| Calendar SoT (BL-031 / US-041) | **Closed / Story accepted 2026-07-19** — Postgres DB `silverman_linkedin_db`; health `calendar_store=postgres:silverman_linkedin_db`, `calendar_store_ready=true` | Prior cutover `/health`; URL via `SILVERMAN_CALENDAR_DATABASE_URL` (secret not recorded here) |
...
- **BL-031 / US-041 Story accepted / BL closed 2026-07-19** (cutover smoke 2026-07-18 evening, `.build_git_sha` `43f773e`): Postgres `silverman_linkedin_db`; worker `SILVERMAN_CALENDAR_DATABASE_URL` set; health `calendar_store_ready=true`. Editorial mount remains durable worker `data/` path (not compartido). Does not restore wiped calendar rows. Fresh LAN verify skipped at close (operator off-network).
