...
| **Service permissions and exposure (BL-026 / US-062 + US-063)** | Operator-facing review of least privilege, listening ports, authentication, allowed mounts/paths, secrets separation (ratifying US-058), and **accepted exposure** — see [service-permissions-and-exposure.md](operations/service-permissions-and-exposure.md). Accepted now: worker `:8010`, n8n `:5678`, and Authority Manager **LAN only**; LinkedIn OAuth callback via Cloudflare public hostname is an **exception only for LinkedIn reauth**; Comfy Cloud + DeepSeek are outbound API clients. Google identity/allowlist on LAN UI = **BL-035 / US-097** (implemented, not Story accepted); **public** console via Cloudflare = **US-099** (deferred). Procedure published ≠ new public surfaces opened. MUST NOT mean `SILVERMAN_LINKEDIN_PUBLICATION_ENABLED` mutation |
| **Accepted exposure (BL-026)** | The operator-approved inventory of what may be reachable and how (LAN vs exceptional public OAuth callback vs outbound clients). Documented in the BL-026 SoT; live evidence uses `blocked` / `confirmed clean` / `finding — remediation required` without secret values. BL-035 extends accepted exposure only when Story accepted for the relevant story: US-097 LAN Google identity; US-099 public UI via Cloudflare Tunnel with worker API remaining private |
| **Operator Google auth (BL-035 / US-097–US-099)** | Activate Google (OIDC) sign-in for Silverman Authority Manager with allowlist `silverio.bernal@gmail.com` and `ltmoralesp84@gmail.com` (US-097); replace browser worker-API-key paste with operator JWT/session for console→API (US-098); expose **UI only** via Cloudflare Tunnel with API staying LAN/private (US-099). **US-097 implemented** (not Story accepted); US-098 / US-099 deferred. n8n keeps API-key HTTP (ADR-0001). MUST NOT mean `SILVERMAN_LINKEDIN_PUBLICATION_ENABLED` mutation or n8n Execute Command |
