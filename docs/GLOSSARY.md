...
| **Warning and test quality baseline (BL-028 / US-067 + US-068)** | Known inventory of suite warnings plus the standing rule that a change MUST NOT introduce **new** warnings attributable to that change — see [warning-and-test-quality-baseline.md](operations/warning-and-test-quality-baseline.md). **Inherited** warnings are listed in dated evidence; **new** warnings are regressions. Suite **failures** are separate quality debt (not “warnings”). Procedure published ≠ CI established (BL-029) |
| **Inherited warning vs new warning** | **Inherited** — documented in the current BL-028 baseline evidence and allowed until remediated. **New** — appears after a change and is not in that baseline (or is a different message class) — must fix or explicitly justify before accepting the change |
