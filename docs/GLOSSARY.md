...
| **Flow B** | AI topic discovery (DeepSeek v1; provider-pluggable later) → AI blog draft in `blog-posts/pending-approval/` → **mandatory operator blog approval** in **Silverman Authority Manager** → promote to `blog-posts/ready/` → same path as Flow A (optional LinkedIn supervision). Weekly **gap sensor** (next local week; gap = 0 LinkedIn posts) may trigger up to 2 drafts | A second mandatory LinkedIn review CMS; revision-history workflow; news-spreader discovery; hand-curated topic backlog as a prerequisite |
