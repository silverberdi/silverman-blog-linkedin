...
| **Business and content metrics (US-053 / US-054 / US-055 / US-056)** | Operator-facing metrics for the content program. **US-053:** blog traffic; LinkedIn reach and engagement (eligible only for **Live on LinkedIn** or an explicitly documented manual-post exception); profile visits and audience growth. **US-054:** recruiter/executive conversations (two-way professional exchange — not one-way likes/reactions); job/consulting opportunities (concrete prospects); high-performing topics/formats (thin operator criteria using US-053 signals where available + optional outcome attribution — not a BI rank from schedule/pending/package-complete). **US-055:** consistent collection per measurement period (value-or-blocked-state completeness; incomplete ≠ measured zero); thin theme/variant comparison from recorded eligible metrics; effective-format identification reusing US-054 §8 — not a second ranking engine, not ranks from schedule metadata. **US-056:** planning-feedback procedure — feed recorded US-053/US-054/US-055 period evidence into future editorial planning; reduce repetition of low-performing content (inverse/complement of §8 / §16; signal or blocked state required); human oversight fail-closed (MUST NOT auto-mutate strategy, editorial backlog, or Flow B without explicit operator action; metrics-log notes ≠ backlog mutation). Normative definition: [business-and-content-metrics.md](operations/business-and-content-metrics.md). Manual-first tracking + durable log template + US-055/US-056 procedures satisfy BL-023 as docs/contract; required auto-fetch / BI dashboard / auto-planning engine remain out of scope. MUST NOT mean Authority Manager triage counts, `flow_a_complete`, blog handoff, or `distribution_scheduled` as measured reach, outcomes, or planning ranks; MUST NOT claim Story accepted by docs alone |
| **Operational metric chips** | Authority Manager at-a-glance publication-triage counts (e.g. upcoming, pending, due soon, deferred, blocked, failed, recently published). Answer “what needs attention in the console,” not US-053/US-054/US-055/US-056 business/content outcomes. Distinct from **business and content metrics** |
...
| **Operational secrets permissions review (US-058)** | Operator-facing procedure to review least-privilege permissions on operational secrets (worker, n8n including `worker_api_key`, Docker/compose mounts, OAuth/token files, deploy keys, related secret-read access) and confirm secrets are absent from Git, logs/HTTP responses, and n8n workflow exports — see [operational-secrets-permissions-review.md](operations/operational-secrets-permissions-review.md). Procedure published ≠ server secrets confirmed clean; Story accepted remains an operator gate after the live review. Independent of US-057 live key rotation (deferred), US-059 ownership/cadence, BL-025 token lifecycle, and BL-026 full exposure. MUST NOT mean Flow A/B behavior changes or `SILVERMAN_LINKEDIN_PUBLICATION_ENABLED` mutation |
| **US-058 review outcome (`blocked` / `confirmed clean` / `finding — remediation required`)** | Per-check (and overall) vocabulary for the US-058 permissions + secrets-absence review: **`blocked`** — check cannot be completed (e.g. no server access); **`confirmed clean`** — check completed with no secret exposure / permissions match expected least privilege; **`finding — remediation required`** — exposure or excessive permission found (record env var **names** / path classes / modes only — never secret values). Incomplete or blocked ≠ confirmed clean. Deferred US-057 rotation ≠ US-058 absence failure by itself |
| **Operational secrets ownership and cadence (US-059)** | Who owns each operational secret class and how often to review/rotate (operator-approved defaults: worker/n8n API key + US-058 re-audit **90 days**; provider keys + LinkedIn client secret **180 days**; deploy key **1 year**; on suspected exposure **immediate**) — see [operational-secrets-ownership-cadence.md](operations/operational-secrets-ownership-cadence.md). Procedure published ≠ keys rotated. Independent of US-057 deferred rotation execution and BL-025 token lifecycle formalization |
| **Service permissions and exposure (BL-026 / US-062 + US-063)** | Operator-facing review of least privilege, listening ports, authentication, allowed mounts/paths, secrets separation (ratifying US-058), and **accepted exposure** — see [service-permissions-and-exposure.md](operations/service-permissions-and-exposure.md). Accepted now: worker `:8010`, n8n `:5678`, and Authority Manager **LAN only**; LinkedIn OAuth callback via Cloudflare public hostname is an **exception only for LinkedIn reauth**; Comfy Cloud + DeepSeek are outbound API clients. Public console + Google auth = future / out of scope. Procedure published ≠ new public surfaces opened. MUST NOT mean `SILVERMAN_LINKEDIN_PUBLICATION_ENABLED` mutation |
| **Accepted exposure (BL-026)** | The operator-approved inventory of what may be reachable and how (LAN vs exceptional public OAuth callback vs outbound clients). Documented in the BL-026 SoT; live evidence uses `blocked` / `confirmed clean` / `finding — remediation required` without secret values |
| **Warning and test quality baseline (BL-028 / US-067 + US-068)** | Known inventory of suite warnings plus the standing rule that a change MUST NOT introduce **new** warnings attributable to that change — see [warning-and-test-quality-baseline.md](operations/warning-and-test-quality-baseline.md). Historical BL-028 snapshot recorded inherited W1 + suite failures; **cleared by BL-033 / US-090**. Suite **failures** are separate quality debt (not “warnings”). Procedure published ≠ CI established (BL-029) |
| **Inherited warning vs new warning** | **Inherited** — documented in the current BL-028/US-090 baseline evidence and allowed until remediated. **New** — appears after a change and is not in that baseline (or is a different message class) — must fix or explicitly justify before accepting the change. After US-090 clearance the clean bar is **0 inherited pytest warnings** |
| **LinkedIn token management (BL-025 / US-060 + US-061)** | Operator SoT for LinkedIn OAuth token lifecycle: secure storage, renewal/expiration, revocation, invalid-token detection, recovery, and development vs production separation — see [linkedin-token-management.md](operations/linkedin-token-management.md). Ratifies existing `linkedin-oauth-token-lifecycle` worker behavior. Healthy status today ≠ no detection/recovery contract; missing refresh token means reauth when access expires. MUST NOT mean forced invalidation or `SILVERMAN_LINKEDIN_PUBLICATION_ENABLED` mutation |
| **Project / runtime context maintenance (BL-030 / US-071 + US-072 + US-073)** | Operator procedure to keep CURRENT-STATE, RUNTIME-STATE, historical/bootstrap docs, and Cursor guidance aligned with reality — see [project-runtime-context-maintenance.md](operations/project-runtime-context-maintenance.md). Depth-C audit + live RUNTIME-STATE refresh recorded in [us-071-us-073-context-maintenance-audit-2026-07-21.md](operations/us-071-us-073-context-maintenance-audit-2026-07-21.md). Procedure published ≠ CI established (**BL-029** remains open). MUST NOT mean `SILVERMAN_LINKEDIN_PUBLICATION_ENABLED` mutation |
| **Continuous integration and controlled deploy promotion (BL-029 / US-069 + US-070 + US-091 + US-092)** | Product intent for GitHub Actions PR validation (tests, specs, YAML/JSON, whitespace, secrets, merge block), a **UAT** worker environment separate from prod, and deploy discipline **PR→UAT then later PR→prod**. Stories are product-documented; workflows/UAT not implemented until an approved OpenSpec change. MUST NOT mean classic GitFlow-only, public console exposure, or mutating prod `SILVERMAN_LINKEDIN_PUBLICATION_ENABLED` as part of CI setup |
| **Branch-per-US integration (BL-038 / US-105)** | From 2026-07-21: every change on a **new branch**; integrate to **`main` only when the owning user story is Story accepted** — see [branch-per-us-integration-policy.md](operations/branch-per-us-integration-policy.md). **Story accepted** 2026-07-21. Complements BL-029; does not replace OpenSpec approval gates |
| **Operator UI / API separation (BL-034 / US-093–095)** | Deploy operator UI as a distinct artifact from the worker API; UAT/prod UI↔API pairing. Product-documented; not implemented until approved OpenSpec change |
| **Google operator login (BL-035 / US-096–098)** | Google/OIDC for human console users + allowlist; machine auth remains for n8n. Builds on US-040D readiness. Not implemented until approved OpenSpec change |
| **X distribution (BL-036 / US-099–101)** | Guarded X (Twitter) distribution assets; blog remains canonical (ADR-0002). Distinct from LinkedIn honesty vocabulary. Not implemented until approved OpenSpec change |
| **Post effectiveness analytics (BL-037 / US-102–104)** | System-backed post metrics and operator-visible effectiveness beyond manual BL-022/023 logs; no auto-mutation of backlog/Flow B. Not implemented until approved OpenSpec change |
