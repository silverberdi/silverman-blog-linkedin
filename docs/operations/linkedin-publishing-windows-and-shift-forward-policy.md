...
**Status:** Policy defined (documentation). Console cadence-conflict warning (**US-087**) is **implemented** (see CURRENT-STATE; not Story accepted). Schedule-time shift-forward (**US-088**) **enforces** the finite scan and feasible-slot rules below in `schedule-linkedin-distribution` / `flow_b_spill_a` (see CURRENT-STATE; not Story accepted / deploy separate). Executable **replan** of already-Scheduled conflicts (**US-089**) is **implemented** via authenticated `POST /replan-linkedin-cadence-conflicts` (see CURRENT-STATE; not Story accepted / deploy separate) and MUST reuse the same shift-forward feasibility helpers — this document does **not** redefine window/horizon numbers. **US-052 / BL-021 Story accepted and backlog closure require operator review beyond this docs change.**
