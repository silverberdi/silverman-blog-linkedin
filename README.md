...
When `SILVERMAN_COMFYUI_IMAGE_ENABLED=true` and generation is required but fails, publish returns `status: failed` with a `blog_image_generation_*` error code and does not write public repo files. HTTP responses include a `blog_image_generation` summary (`status`, paths, requested `width`/`height`, `prompt_hash`, and when applicable `workflow_controls_dimensions`) but never full prompt text or ComfyUI API keys. `workflow_controls_dimensions` is `true` only when the workflow template exposes width/height bindings; Comfy Cloud preset workflows without bindings report `false` even though actual PNG size may differ (for example 1536×1024). The worker calls ComfyUI REST endpoints (`/prompt`, `/view`, and job status via `/jobs/{id}` on Comfy Cloud or `/history/{id}` locally), optionally prefixed for hosted deployments. Comfy Cloud `/view` redirects are followed automatically; signed storage URLs are not logged or stored in metadata.
...
Phases 1–4 are blocked until Phase 0 passes (use `--force` for debugging only). Phase 0 ancestry defaults (`88cd5bc`, `96519c3`, `9dba064`) gate validated operational capabilities — distinct from `last_verified_baseline` in [docs/CURRENT-STATE.md](docs/CURRENT-STATE.md) (point-in-time snapshot, not a permanent runtime requirement). Override defaults with repeatable `--expected-commit` (replaces defaults entirely; no merge).
...
| Purpose | Default host path | Container mount |
|---------|-------------------|-----------------|
| Editorial workspace (blog posts, metadata, LinkedIn drafts) | `/home/silverman/compartido_mac/silverman-blog-linkedin` | `/data/silverman-blog-linkedin` |
| Public GitHub Pages repo (`silverberdi.github.io`) | `/home/silverman/silverberdi.github.io` | `/public-blog` |

The public repo checkout must contain `_posts/` and `assets/images/` before Flow A publish smoke. `deploy-worker.sh` verifies this before `docker compose up` (set `SKIP_PUBLIC_BLOG_REPO_CHECK=1` only for non-publishing deploys). The deploy script does **not** clone the GitHub Pages repo automatically — clone or sync it manually, then redeploy. If publish validation passes but publishing fails with `blog_publish_public_repo_not_configured`, the worker container is missing the public repo mount or env (`SILVERMAN_GITHUB_PAGES_REPO_PATH=/public-blog`) — this is a worker deployment issue, not an n8n failure.
